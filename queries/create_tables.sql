...
CREATE TABLE SHIPMENT_REQUEST(
    ...
    CONSTRAINT STATUS_CHECK CHECK (STATUS='PENDING' OR STATUS='PROCESSED' OR STATUS='PROCESSING')
);
...
-- FOR EACH ROW
-- DECLARE
--   v_hashed_password RAW(256); -- Adjust size based on hashing algorithm (e.g., SHA-256)
-- BEGIN
--   -- Hash the password using DBMS_CRYPTO
--   v_hashed_password := DBMS_CRYPTO.HASH(UTL_RAW.CAST_TO_RAW(:NEW.USER_PASSWORD), DBMS_CRYPTO.HASH_SH256);

--   -- Store the hashed password in the new row
--   :NEW.Password := v_hashed_password;
-- END;
-- /
